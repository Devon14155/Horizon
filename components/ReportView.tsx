...
export const ReportView: React.FC = () => {
  ...
  const SimpleBarChart = ({ data }: { data: ChartData }) => {
    ...
    return (
      <div className="my-8 p-6 bg-horizon-900/50 rounded-xl border border-horizon-700">
        <h4 className="text-center font-bold text-gray-200 mb-6">{data.title}</h4>
        <div className="flex items-end justify-around h-48 gap-2">
          {data.datasets[0].data.map((value, i) => (
            <div key={i} className="flex flex-col items-center flex-1 group">
               <div className="text-xs text-gray-400 mb-1 opacity-0 group-hover:opacity-100 transition-opacity">{value}</div>
               <div 
  ...
  return (
    <div className="absolute inset-0 z-20 bg-horizon-900 flex flex-col animate-in slide-in-from-bottom-10 duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-horizon-900">
        <div className="max-w-4xl mx-auto bg-horizon-800 p-8 md:p-12 rounded-xl shadow-xl border border-horizon-700">
          <h1 className="text-3xl font-bold text-white mb-2 font-mono">{session.title}</h1>
          <p className="text-gray-400 mb-8 border-b border-horizon-700 pb-4 flex justify-between items-center">
          ...
          {uniqueSourceUrls.length > 0 && (
            <div className="mt-16 pt-8 border-t border-horizon-700 page-break-inside-avoid">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-semibold text-white">References</h3>
                <div className="flex bg-horizon-900 rounded-lg p-1 border border-horizon-700">
                  {(['APA', 'MLA', 'CHICAGO'] as CitationStyle[]).map(style => (
                    <button
                      key={style}
                      onClick={() => setCitationStyle(style)}
                      className={`px-3 py-1 text-xs rounded-md transition-colors ${citationStyle === style ? 'bg-horizon-600 text-white' : 'text-gray-400 hover:text-gray-200'}`}
                    >
                      {style}
                    </button>
                  ))}
              ...
              <ul className="space-y-4">
                {uniqueSourceUrls.map((url, i) => (
                  <li key={i} className="text-sm text-gray-300 bg-horizon-900/50 p-3 rounded-lg border border-horizon-700/50">
